...
/* Stateless bulk vertex decoding, shared by the MeshSet reader and engine
 * bindings. Does not open an install, allocate, remap UVs or change coordinates.
 * first is the byte offset of the first element, stride is bytes per vertex,
 * count is vertices, format is VertexElementFormat (the existing MeshSet values).
 * Returns components per vertex (1..4), or 0 for unsupported/invalid input.
 * out==NULL validates the input and queries components without writing.
 * Otherwise out_capacity is FLOATS, not bytes, and must hold count*components.
 * Invalid inputs write nothing. Source and output must not overlap. No pointers
 * are retained. Concurrent calls on independent buffers are supported.
 */
BF6_API int bf6_decode_vertex_attribute(const uint8_t* data, int64_t data_bytes,
...
/* THE TEXTURES A VARIATION ACTUALLY WEARS, FOUND BY NAME.
 *
 * A variation's textures are NOT referenced by the variation database. Verified
 * across the whole dump: 0 of 1,013 meshvariationdb files contain the Cairo
 * vst_/t_ texture partition guids, the ObjectVariation asset is a ~326-byte
 * stub of {NameHash, Name} with zero imports, and only ~44 of 7,191 variation
 * entries carry explicit texture parameters. The database gives (mesh,
 * variation); the sheets are found by the name convention
 *
 *     vst_<ObjectVariation basename>_<suffix>
 *
 * searched in the ObjectVariation's folder and the two above it, because the
 * sheet commonly sits one level above the `ov/` subfolder holding the asset.
 *
 * Why it is needed rather than nice to have: on MP_Aftermath every member of
 * pf_portal_gcarsuv_01_policeusny reports bf6_variation_live == 0 even though
 * the walk supplies a real ObjectVariation path and the placing bundle does own
 * a shader state depot. The derived key resolves in no scope, so a scoped read
 * falls back to the base key and the police car renders as a plain SUV - while
 * vst_ov_com_carsuv_01_policeusny_cs sits in the mount, measured 2026-09-12.
 *
 * `suffix` and `res_name` point into the context and are valid until
 * bf6_close. `texture` is an id for bf6_texture_at, or -1. `slot` is the
 * bf6_tex_slot the suffix maps to, or -1 where the token has no settled meaning
 * (`ca`); bind the known slots and leave the rest alone. A convention, not a
 * guarantee.
 *
 * CANDIDATE DISCOVERY, NOT A MATERIAL RECIPE. It reports every sheet whose
 * name fits the convention. It does NOT say which section of a mesh the sheet
 * belongs to, and several suffixes map to the same generic slot, so a caller
 * that binds all of them to every section will put body paint on trim and
 * interiors. Resolve per section against the authored records and use this only
 * where that leaves a hole.
 *
 * Two-step like the rest of the API: call with out=NULL for the count. A
 * non-empty result is cached on the context per ObjectVariation path; a miss is
 * deliberately NOT cached, because the mount grows and a remembered negative
 * would outlive the mount that would have satisfied it. */
typedef struct {
    const char* suffix;    /* the convention token that matched, e.g. "cs"    */
    const char* res_name;  /* the texture resource it resolved to             */
    int32_t     texture;   /* id for bf6_texture_at, or -1                    */
    int32_t     slot;      /* bf6_tex_slot, or -1 when the token is unsettled */
} bf6_variation_texture;

BF6_API int bf6_variation_textures(bf6_ctx*, const char* variation,
...
/* Exact native-lattice texture samples; max_size 64..2048; source_size optional. */
BF6_API bf6_terrain* bf6_read_terrain_preview(bf6_ctx*, const char* level,
...
/* WHO SITS WHERE. Seat 0 is the driver's. A vehicle opens with the driver seated;
...
typedef struct {
    float position[3];
    float look_at[2][3];
    float position_rotation_degrees[3];
    float look_at_rotation_degrees[2][3];
    float focal_length_mm;
    float aperture;
    float shutter_speed;
    float focus_distance;
 ...
 * `channel` addresses the decoder's float4 output and `lane` addresses its
 * component. Quaternion and Vector3 DOFs always use lane zero. DCT scalar
 * DOFs are packed four per float4, while VBR scalar DOFs use one float4 each.
...
/* Ordered identity table for the shared weapon skin palette. Weapon mesh
 * BoneIndices address this exact 66-row order; the same case-sensitive names
 * occur on the soldier rig and are the join needed to animate weapon parts
 * with a soldier clip. */
typedef struct {
    char    name[64];
    int32_t index;
} bf6_weapon_bone_info;

BF6_API int bf6_weapon_bones(bf6_ctx*, bf6_weapon_bone_info* out,
                             int out_max);
