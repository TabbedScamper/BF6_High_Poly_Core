...
#include <set>
#include <cstring>
#include <cctype>          // std::tolower, for the variation name convention
#include <exception>
#include <new>
#include <filesystem>
#include <functional>
...
struct bf6_ctx {
    ...
    // bf6_variation_textures answers, keyed by the ObjectVariation path. The
    // rows hand out const char*, so the strings must outlive the call and must
    // not move: std::map never relocates a mapped value once inserted, which a
    // vector of vectors would, invalidating pointers already returned.
    struct VarTexRow { std::string suffix, res; int32_t texture, slot; };
    std::map<std::string, std::vector<VarTexRow>> var_tex;
...
static bf6_terrain* read_terrain_block(bf6_ctx* c, const char* level, bool water_surface,
                                      int preview_limit = 0, int* source_size = nullptr) {
    ...
    bf6::TerrainGrid g;
    const int native_size = std::min(t.native_size(), 16385);
    if (source_size) *source_size = native_size;
    if (preview_limit) {
        if (!t.composite_sampled(g, std::clamp(native_size / 4, 64, preview_limit), err)) return nullptr;
    } else if (!t.composite(g, 0, err)) return nullptr;
...
bf6_terrain* bf6_read_terrain_preview(bf6_ctx* c, const char* level,
                                      int water_surface, int max_size, int* source_size) {
    if (source_size) *source_size = 0;
    if (max_size < 64 || max_size > 2048 || (water_surface != 0 && water_surface != 1)) return nullptr;
    return read_terrain_block(c, level, water_surface != 0, max_size, source_size);
}
...
// ---- a variation's textures, which the database does not reference ---------
//
// GROUND TRUTH, AND IT IS COUNTER-INTUITIVE. A variation's textures are not
// referenced by the variation database at all. The Godot reader established
// this across the whole dump (bf6_mvdb.gd): 0 of 1,013 meshvariationdb files
// contain the Cairo vst_/t_ texture partition guids, the ObjectVariation asset
// is a ~326-byte stub of {NameHash, Name} with zero imports, the mesh
// partition imports only its shader, and only ~44 of 7,191 variation entries
// carry explicit texture parameters. So the database gives (mesh, variation)
// and the textures are found BY NAME: vst_<ov basename>_<suffix>.
//
// WHY THIS HAS TO EXIST. bf6_variation_live is 0 for every member of
// pf_portal_gcarsuv_01_policeusny on MP_Aftermath even though the walk hands
// over a real ObjectVariation path and the placing bundle does own a shader
// state depot. The derived key resolves in no scope, so bf6_read_mesh_scoped
// falls back to the base key and a police car renders as a plain SUV. Meanwhile
// the sheet exists and is reachable:
//
//   common/environment/generic/common/props/carsuv_01/
//       vst_ov_com_carsuv_01_policeusny_cs
//
// measured 2026-09-12 through this library. Only the naming rule was missing,
// so only the naming rule is added.
//
// A CONVENTION, NOT A GUARANTEE. The suffix set and the slot mapping are the
// ones the Godot reader verified; `ca` has no settled meaning so its slot is
// reported as -1 rather than guessed. A caller binds the sheets whose slot is
// known and leaves the rest alone.
static int32_t variation_suffix_slot(const std::string& s)
{
    if (s == "cs" || s == "c")                              return BF6_TEX_ALBEDO;
    if (s == "nm" || s == "nmo" || s == "nea" || s == "nsm") return BF6_TEX_NORMAL;
    if (s == "mask" || s == "a")                            return BF6_TEX_MASK;
    return -1;
}

int bf6_variation_textures(bf6_ctx* c, const char* variation,
                           bf6_variation_texture* out, int out_max)
{
    if (!c || !variation || !*variation) return 0;

    std::string ov = variation;
    for (char& ch : ov) ch = (char)std::tolower((unsigned char)ch);

    auto cached = c->var_tex.find(ov);
    if (cached == c->var_tex.end())
    {
        const size_t slash = ov.find_last_of('/');
        std::string dir  = (slash == std::string::npos) ? std::string() : ov.substr(0, slash);
        std::string name = (slash == std::string::npos) ? ov : ov.substr(slash + 1);
        if (name.size() > 4 && name.compare(name.size() - 4, 4, ".ebx") == 0)
            name.resize(name.size() - 4);

        // THE SHEET IS NOT IN THE ov/ FOLDER. Measured: the ObjectVariation
        // lives in .../props/carsuv_01/ov/ and its sheet in
        // .../props/carsuv_01/, one level up. The ov's own folder and its
        // grandparent are tried too, so the rule does not depend on every
        // authoring tree using an ov/ subfolder.
        std::vector<std::string> dirs;
        if (!dir.empty())
        {
            dirs.push_back(dir);
            const size_t up1 = dir.find_last_of('/');
            if (up1 != std::string::npos)
            {
                dirs.push_back(dir.substr(0, up1));
                const size_t up2 = dirs.back().find_last_of('/');
                if (up2 != std::string::npos) dirs.push_back(dirs.back().substr(0, up2));
            }
        }

        static const char* kSuffixes[] =
            { "cs", "nea", "nm", "nmo", "a", "c", "nsm", "mask", "ca" };

        // A MISS IS NOT AN ANSWER WORTH KEEPING.
        //
        // Built into a local first and only memoised when something was found.
        // The mount grows: bf6_mount_level_archives and bf6_mount_all can both
        // add the very resources this was looking for, and a cached "no sheets"
        // keyed only by the OV path would outlive the mount that would have
        // satisfied it. Remembering a negative here is remembering that the
        // catalogue used to be smaller.
        //
        // A repeated miss costs a handful of has_res hash lookups, which is the
        // right price for not being wrong.
        std::vector<bf6_ctx::VarTexRow> found;
        for (const char* suf : kSuffixes)
        {
            const std::string leaf = "vst_" + name + "_" + suf;
            for (const std::string& d : dirs)
            {
                const std::string cand = d + "/" + leaf;
                if (!c->src.has_res(cand)) continue;
                bf6_ctx::VarTexRow r;
                r.suffix  = suf;
                r.res     = cand;
                r.texture = bf6_texture_id_by_name(c, cand.c_str());
                r.slot    = variation_suffix_slot(suf);
                found.push_back(std::move(r));
                break;   // the first directory that has it wins
            }
        }
        if (found.empty()) { return 0; }
        c->var_tex[ov] = std::move(found);
        cached = c->var_tex.find(ov);
    }

    const std::vector<bf6_ctx::VarTexRow>& rows = cached->second;
    const int n = (int)rows.size();
    if (!out || out_max <= 0) return n;
    const int give = n < out_max ? n : out_max;
    for (int i = 0; i < give; i++)
    {
        out[i].suffix   = rows[i].suffix.c_str();
        out[i].res_name = rows[i].res.c_str();
        out[i].texture  = rows[i].texture;
        out[i].slot     = rows[i].slot;
    }
    return n;
}
...
int bf6_prepare_level(bf6_ctx* c, const char* level, char* err, int err_len)
{
    std::string why;
    if (c) c->src.set_progress([c](const char* stage, int done, int total) {
        return c->report(stage, done, total);
    });
    if (!ensure_mounted(c, level, why) || !ensure_types(c, why)) {
...
void bf6_free(bf6_ctx* c, void* handle) {
    ...
    case bf6_ctx::HK_ANTRT: bf6__antrt_delete(handle); break;
    default: break;
    }
}
...
#include "ant_runtime_ext.inc"
#include "inspect_input_ext.inc"
