...
bool CasLocator::open(const std::string& game_dir, std::string& err) {
    // Reopen is a session boundary; stale missing paths cannot survive it.
    paths_.clear();
    by_index_.clear();
    const char* compare = std::getenv("BF6_CAS_UNCACHED_PATHS");
    uncached_paths_ = compare && std::string(compare) == "1";
    game_ = game_dir;
...
std::string CasLocator::cas_path(uint32_t install_chunk_id, int cas_index) const {
    if (uncached_paths_) return resolve_path(install_chunk_id, cas_index);
    const auto key = std::make_pair(install_chunk_id, cas_index);
    {
        std::shared_lock<std::shared_mutex> lock(paths_mutex_);
        auto found = paths_.find(key);
        if (found != paths_.end()) return found->second;
    }
    std::unique_lock<std::shared_mutex> lock(paths_mutex_);
    auto found = paths_.find(key);
    if (found != paths_.end()) return found->second;
    return paths_.emplace(key, resolve_path(install_chunk_id, cas_index)).first->second;
}

std::string CasLocator::resolve_path(uint32_t install_chunk_id, int cas_index) const {
