...
private:
    ...
    // Resolve each physical archive once per opened install. Asset requests
    // share the result rather than stat'ing Data/Update paths for every row.
    mutable std::shared_mutex paths_mutex_;
    mutable std::map<std::pair<uint32_t, int>, std::string> paths_;
    bool uncached_paths_ = false;
    std::string resolve_path(uint32_t install_chunk_id, int cas_index) const;
};
