...
// One implementation for native mesh reads and Godot's bulk decoder. Check the
// full input/output spans before writing; subtraction avoids overflow in the
// last-element calculation for malformed offsets/strides.
static int decode_vertex_attribute(const uint8_t* buf, int64_t buflen,
    int64_t first, int64_t stride, int32_t count, int32_t fmt,
    float* out, int64_t capacity) {
    const int size = fmt_size(fmt), comps = fmt_components(fmt);
    if (!buf || buflen < 0 || first < 0 || stride <= 0 || count <= 0
        || !size || !comps || fmt == 50 || capacity < 0) return 0;
    if (first > buflen || size > buflen - first
        || int64_t(count - 1) > (buflen - first - size) / stride) return 0;
    if (!out) return comps;
    if (capacity < int64_t(count) * comps) return 0;
    for (int i = 0; i < count; i++) {
        const size_t p = size_t(first + int64_t(i) * stride);
        const size_t o = size_t(i) * comps;
        switch (fmt) {
...
static std::pair<std::vector<float>, int> read_attr(
    ...
    const int fmt = el[1], off = el[2], si = el[3];
    if (base < 0 || count <= 0 || si >= (int)streams.size()) return {out, 0};
    int64_t first = int64_t(base) + off;
    for (int s = 0; s < si; s++) first += int64_t(streams[s][0]) * count;
    const int stride = streams[si][0];
    const int comps = decode_vertex_attribute(buf, int64_t(buflen), first, stride, count, fmt, nullptr, 0);
    if (!comps) return {out, 0};
    out.resize(size_t(count) * comps);
    decode_vertex_attribute(buf, int64_t(buflen), first, stride, count, fmt, out.data(), int64_t(out.size()));
    return {out, comps};
}
...
extern "C" BF6_API int bf6_decode_vertex_attribute(const uint8_t* data,
    int64_t data_bytes, int64_t first, int64_t stride, int32_t count,
    int32_t format, float* out, int64_t out_capacity) {
    return bf6::decode_vertex_attribute(data, data_bytes, first, stride,
                                       count, format, out, out_capacity);
}
