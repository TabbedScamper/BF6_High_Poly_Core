...
// composite() and composite_sampled() share everything below, so the compact
// path evaluates the very same expressions at each lattice point it keeps and
// the two cannot drift apart when one of them is edited.
struct CompositeFrame {
    float lo[3] = { 1e30f, 1e30f, 1e30f };
    float hi[3] = { -1e30f, -1e30f, -1e30f };
    float span_x = 0.f, span_z = 0.f;
    std::vector<const TerrainNode*> with;
};

bool composite_frame(const std::vector<TerrainNode>& nodes, CompositeFrame& f)
{
    for (const TerrainNode& n : nodes)
    {
        for (int i = 0; i < 3; i++)
        {
            f.lo[i] = std::min(f.lo[i], n.lo[i]);
            f.hi[i] = std::max(f.hi[i], n.hi[i]);
        }
        if (!n.values.empty()) f.with.push_back(&n);
    }
    if (f.with.empty()) return false;

    // Deepest last, so the finest data available wins everywhere without
    // needing to know which levels a map happens to have.
    std::stable_sort(f.with.begin(), f.with.end(),
        [](const TerrainNode* a, const TerrainNode* b) { return a->depth < b->depth; });

    f.span_x = std::max(0.001f, f.hi[0] - f.lo[0]);
    f.span_z = std::max(0.001f, f.hi[2] - f.lo[2]);
    return true;
}

// A node's footprint on a size*size grid, half-open [x0,x1) x [z0,z1).
struct NodeRect {
    int   x0 = 0, x1 = 0, z0 = 0, z1 = 0;
    float xspan = 1.f, zspan = 1.f;
    bool  xmany = false, zmany = false;
};

bool node_rect(const TerrainNode& n, const CompositeFrame& f, int size, NodeRect& r)
{
    int x0 = (int)std::floor((n.lo[0] - f.lo[0]) / f.span_x * (float)size);
    int x1 = (int)std::ceil ((n.hi[0] - f.lo[0]) / f.span_x * (float)size);
    int z0 = (int)std::floor((n.lo[2] - f.lo[2]) / f.span_z * (float)size);
    int z1 = (int)std::ceil ((n.hi[2] - f.lo[2]) / f.span_z * (float)size);
    ...
    r.zspan = (float)std::max(1, z1 - z0 - 1);
    r.xspan = (float)std::max(1, x1 - x0 - 1);
    r.zmany = z1 - z0 > 1;
    r.xmany = x1 - x0 > 1;
    return true;
}

// Grid offset i inside a node's footprint -> sample index and fraction.
inline void lattice_axis(int i, bool many, float span, int s0, float ssp, float hi_f,
                         int s_max, int& si, float& t)
{
    const float fr = many ? (float)i / span : 0.f;
    float       sf = (float)s0 + fr * ssp;
    sf = std::clamp(sf, 0.f, hi_f);
    si = (int)sf;
    if (si > s_max) si = s_max;
    t = sf - (float)si;
}

// BILINEAR, not nearest. The pyramid is concentric, so away from the middle a
// coarse node is upsampled many times into the grid and nearest turns every
// outskirt into stair steps. False leaves the destination untouched.
inline bool bilerp_sample(const std::vector<uint16_t>& v, size_t r0, int xs,
                          float txc, float tz, uint16_t& dst)
{
    const size_t r1 = r0 + (size_t)xs;
    if (r1 + 1 >= v.size()) return false;
...
bool Terrain::composite(TerrainGrid& out, int size, std::string& err) const
{
    ...
    if (!composite_frame(nodes_, frame)) { err = "no nodes carry height values"; return false; }

    out.size = size;
    out.heights.assign((size_t)size * (size_t)size, 0);
    for (int i = 0; i < 3; i++) { out.lo[i] = frame.lo[i]; out.hi[i] = frame.hi[i]; }
    ...
    std::vector<int>   col_sx;
    std::vector<float> col_tx;
    for (const TerrainNode* n : frame.with)
    {
        NodeRect r;
        if (!node_rect(*n, frame, size, r)) continue;
        ...
        const int ncol = r.x1 - r.x0;
        col_sx.resize((size_t)ncol);
        col_tx.resize((size_t)ncol);
        for (int ci = 0; ci < ncol; ci++)
            lattice_axis(ci, r.xmany, r.xspan, s0, ssp, hi_f, sx_max,
                         col_sx[(size_t)ci], col_tx[(size_t)ci]);

        for (int gz = r.z0; gz < r.z1; gz++)
        {
            int sy0 = 0; float tz = 0.f;
            lattice_axis(gz - r.z0, r.zmany, r.zspan, s0, ssp, hi_f, sx_max, sy0, tz);

            const size_t rowb  = (size_t)sy0 * (size_t)xs_;
            const size_t dbase = (size_t)gz * (size_t)size + (size_t)r.x0;

            for (int ci = 0; ci < ncol; ci++)
                bilerp_sample(v, rowb + (size_t)col_sx[(size_t)ci], xs_, col_tx[(size_t)ci], tz,
                              out.heights[dbase + (size_t)ci]);
        }
    }
    return true;
}

bool Terrain::composite_sampled(TerrainGrid& out, int sample_size, std::string& err) const
{
    err.clear();
    // The lattice being subsampled is exactly the one composite(out, 0) builds.
    const int full = std::min(native_size(), 16385);
    if (full <= 0) { err = "no nodes carry height values"; return false; }
    if (sample_size <= 0 || sample_size > 16385)
    { err = "invalid compact terrain sample size"; return false; }
    const int64_t F = full, N = sample_size;

    CompositeFrame frame;
    if (!composite_frame(nodes_, frame)) { err = "no nodes carry height values"; return false; }

    out.size = sample_size;
    out.heights.assign((size_t)N * (size_t)N, 0);
    for (int i = 0; i < 3; i++) { out.lo[i] = frame.lo[i]; out.hi[i] = frame.hi[i]; }
    out.world_size_y = world_size_y_;
    ...
    // Compact index k lands on full index k*F/N, monotone in k. A full-grid
    // half-open span [a,b) is therefore hit by exactly k in
    // [ceil(a*N/F), ceil(b*N/F)).
    auto full_of = [&](int64_t k) { return (int)(k * F / N); };
    auto first_k = [&](int64_t a) { return (int)std::min(N, (a * N + F - 1) / F); };
    ...
    {
        NodeRect r;
        if (!node_rect(*n, frame, full, r)) continue;
        const int ka = first_k(r.x0), kb = first_k(r.x1);
        const int ja = first_k(r.z0), jb = first_k(r.z1);
        if (kb <= ka || jb <= ja) continue;
        const std::vector<uint16_t>& v = n->values;

        const int ncol = kb - ka;
        col_sx.resize((size_t)ncol);
        col_tx.resize((size_t)ncol);
        for (int c = 0; c < ncol; c++)
            lattice_axis(full_of(ka + c) - r.x0, r.xmany, r.xspan, s0, ssp, hi_f, sx_max,
                         col_sx[(size_t)c], col_tx[(size_t)c]);

        for (int j = ja; j < jb; j++)
        {
            int sy0 = 0; float tz = 0.f;
            lattice_axis(full_of(j) - r.z0, r.zmany, r.zspan, s0, ssp, hi_f, sx_max, sy0, tz);

            const size_t rowb  = (size_t)sy0 * (size_t)xs_;
            const size_t dbase = (size_t)j * (size_t)N + (size_t)ka;

            for (int c = 0; c < ncol; c++)
                bilerp_sample(v, rowb + (size_t)col_sx[(size_t)c], xs_, col_tx[(size_t)c], tz,
                              out.heights[dbase + (size_t)c]);
        }
    }
    return true;
}
