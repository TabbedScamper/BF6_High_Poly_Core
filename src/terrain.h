...
public:
    ...
    // Exactly composite(full, 0) followed by the integer nearest subsample
    //   out[y*N + x] = full[(y*F/N)*F + x*F/N],  F = full.size, N = sample_size
    // bit for bit, with the same bounds and height scale. Only the N*N lattice
    // points are evaluated, so the F*F native composite is never allocated or
    // painted. This is NOT a rescaled grid: it keeps the native lattice's
    // nearest samples. Node resolution (resolve_external) is unchanged; this
    // saves composite memory and work only.
    bool composite_sampled(TerrainGrid& out, int sample_size, std::string& err) const;
