...
int main(int argc, char** argv)
{
    ...
    std::set<std::string> live_mode_names;
    for (const bf6_armory_camera_mode& mode:inventory)
        live_mode_names.insert(mode.mode);
    static const char* category_controls[] = {
        "Sight", "OpticAccessory", "Barrel", "Magazine", "Flashlight",
        "Rangefinder", "Muzzle", "Laser", "UnderBarrel", "Ammunition",
        "Launcher", "LeftRail"
    };
    int category_routes=0;
    std::set<std::string> distinct_category_modes;
    for (const char* category:category_controls) {
        const char* route=armory_camera::attachment_mode_for_category(category);
        if (route && live_mode_names.count(route)==1) {
            ++category_routes;
            distinct_category_modes.insert(route);
        }
    }
    const bool fake_category_route=
        armory_camera::attachment_mode_for_category("__fabricated_category")!=nullptr;
    std::printf("category camera routes=%d/%zu distinct=%zu fake=%d\n",
        category_routes,sizeof(category_controls)/sizeof(category_controls[0]),
        distinct_category_modes.size(),fake_category_route?1:0);
    ...
    // Frostbite is right-handed: when view-forward is world +Z, camera-local
    // +X is world -X and local +Z is camera-backward.
    // Exercise that bridge independently of live content so an accidental
    // cross-product/sign reversal cannot still pass on internally coherent
    // game data.
    armory_camera::Input axis_input{};
    axis_input.mode.focal_length_mm=50.f;
    axis_input.mode.post_offset[0]=1.f;
    axis_input.mode.post_offset[2]=2.f;
    axis_input.anchor.position={0.f,0.f,0.f};
    axis_input.look_at.position={0.f,0.f,10.f};
    axis_input.sensor_width_mm=36.f;
    axis_input.sensor_height_mm=24.f;
    axis_input.viewport_width=1920.f;
    axis_input.viewport_height=1080.f;
    axis_input.post_law=armory_camera::PostOffsetLaw::CameraDolly;
    armory_camera::Output axis_output{};
    const bool axis_ok=armory_camera::compose(axis_input,axis_output);
    armory_camera::Vec3 axis_origin_screen{};
    const bool axis_projected=axis_ok && armory_camera::project(
        axis_output,{0.f,0.f,10.f},0.f,0.f,1920.f,1080.f,
        axis_origin_screen);
    const bool axis_convention=axis_projected &&
        std::fabs(axis_output.eye.x+1.f)<1e-5f &&
        std::fabs(axis_output.eye.z+2.f)<1e-5f &&
        axis_origin_screen.x<960.f;
    armory_camera::Input world_input=axis_input;
    world_input.post_space=armory_camera::PostOffsetSpace::World;
    armory_camera::Output world_output{};
    const bool world_ok=armory_camera::compose(world_input,world_output) &&
        std::fabs(world_output.eye.x-1.f)<1e-5f &&
        std::fabs(world_output.eye.z-2.f)<1e-5f;
    ...
    bf6_armory_camera_mode sight_mode{};
    const bool sight_mode_ok=bf6_armory_camera_mode_read(c,"weaponsightbehavior",&sight_mode)!=0;
    armory_camera::Input sight_input{};
    sight_input.mode=sight_mode;
    sight_input.anchor=scope;
    sight_input.look_at=align;
    sight_input.weapon_pre_offset=m4_pre;
    sight_input.weapon_post_offset=m4_post;
    sight_input.sensor_width_mm=sensor.width_mm;
    sight_input.sensor_height_mm=sensor.height_mm;
    sight_input.viewport_width=1920.f;
    sight_input.viewport_height=1080.f;
    sight_input.post_law=armory_camera::PostOffsetLaw::LookAtTarget;
    armory_camera::Output sight_output{};
    const bool sight_compose_ok=sight_mode_ok && armory_camera::compose(sight_input,sight_output);
    std::printf("sight real=%d composed=%d eye=(%.6f %.6f %.6f) target=(%.6f %.6f %.6f) autofocus=%.6f aperture=%.6f\n",
        sight_mode_ok?1:0,sight_compose_ok?1:0,
        sight_output.eye.x,sight_output.eye.y,sight_output.eye.z,
        sight_output.focus_target.x,sight_output.focus_target.y,sight_output.focus_target.z,
        sight_output.autofocus_distance,sight_mode.aperture);
    ...
    const bool pass=sensor_ok && sensor.candidates>=1 && real && !fabricated &&
        ...
        category_routes==(int)(sizeof(category_controls)/sizeof(category_controls[0])) &&
        distinct_category_modes.size()>=8 && !fake_category_route &&
        null_dolly_ok && null_target_ok && null_law_delta<1e-6f && !invalid_ok &&
        axis_convention && world_ok &&
        sight_mode_ok && sight_compose_ok &&
