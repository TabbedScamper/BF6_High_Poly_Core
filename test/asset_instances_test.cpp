...
int main(int argc, char** argv)
{
    ...
    for (const bf6_instance& r : rows) {
        if ((std::strcmp(argv[3], "*") == 0) ||
            ...
            real++;
            if (real <= 256)
                std::printf("match (%8.2f %7.2f %8.2f) "
                    "basis=[%.5f %.5f %.5f | %.5f %.5f %.5f | %.5f %.5f %.5f] "
                    "res=%s source=%s\n",
                    r.xform[9], r.xform[10], r.xform[11],
                    r.xform[0], r.xform[1], r.xform[2],
                    r.xform[3], r.xform[4], r.xform[5],
                    r.xform[6], r.xform[7], r.xform[8],
