...
int main(int argc, char** argv)
{
    ...
    for (const bf6_attachment_catalogue_row& r : rows)
        if (std::strcmp(r.slot, "scp") == 0)
            std::printf("scope-row\t%d\t%s\t%s\t%s\t%s\n",
                        r.order, r.name, r.detail_title, r.name_key,
                        r.ad_stem);
     ...
     * which is the same token the roster is built from. */
    {
        ...
        if (wn > 0)
        {
            ...
            for (const bf6_weapon_name_row& row : names)
                if (std::strcmp(row.weapon, "xm7") == 0 ||
                    std::strcmp(row.weapon, "brod3") == 0 ||
                    std::strcmp(row.weapon, "bren3") == 0)
                    std::printf("  current-carbine %-8s -> %-26s [%s]\n",
                                row.weapon, row.name, row.class_label);
