 ...
 *
 *   ebx_test <game_dir> <toc|@frontend> <exe> list
 *   ebx_test <game_dir> <toc|@frontend> <exe> dump <name> <out>
 *   ebx_test <game_dir> <toc|@frontend> <exe> read <name>
 *
...
int main(int argc, char** argv)
{
    ...
    {
        std::fprintf(stderr,
            "usage: ebx_test <game_dir> <toc|@frontend> <exe> list\n"
            "       ebx_test <game_dir> <toc|@frontend> <exe> dump <name> <out>\n"
            "       ebx_test <game_dir> <toc|@frontend> <exe> read <name>\n");
        return 2;
    }
    ...
    const bool mounted = std::strcmp(argv[2], "@frontend") == 0
        ? src.mount_frontend(err)
        : src.mount_toc(argv[2], err);
