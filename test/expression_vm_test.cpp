...
static const uint32_t kSubtract = 0x13579bdfu;
...
class OrderingHost final : public bf6::expression::Host {
public:
    bool describe(uint32_t key, bf6::expression::OperatorSignature& out) override {
        if (key != kSubtract) return false;
        out.input_widths = {4, 4};
        out.output_width = 4;
        return true;
    }
    ...
    bool invoke(uint32_t key, const std::vector<bf6::expression::Value>& args,
                bf6::expression::Value& out) override {
        if (key != kSubtract || args.size() != 2 ||
            !args[0].known || !args[1].known) return false;
        out = bf6::expression::Value::from_u32(
            args[0].as_u32() - args[1].as_u32());
        return true;
    }
};
...
bf6::expression::Graph inline_topup_graph() {
    using namespace bf6::expression;
    Graph g;
    g.exact_record_tiling = true;
    g.header.slot_file_size = 16;
    g.constant_pool = {10, 0, 0, 0};
    Record subtract;
    subtract.offset = 0; subtract.next = 28; subtract.kind = 0x02;
    subtract.has_operator = true; subtract.operator_key = kSubtract;
    // Raw order deliberately puts the inline value first.  Frostbite passes
    // declared operands first and uses this pair only to top up to arity.
    subtract.operands = {{3, 2}, {0, 0}, {2, 4}};
    Record done;
    done.offset = 28; done.kind = 0x2c;
    g.records = {subtract, done};
    return g;
}
...
int main() {
    ...
    OrderingHost ordering_host;
    const Evaluation ordering = evaluate(inline_topup_graph(), nullptr, {},
                                         &ordering_host);
    ...
    std::printf("  declared-before-inline ordering: %u\n",
                ordering.result.as_u32());
    ...
    return real.result.known && !real.result.tainted &&
           ...
           ownership_control.termination == Termination::InvalidGraph &&
           ordering.result.known && !ordering.result.tainted &&
           ordering.result.as_u32() == 8 ? 0 : 1;
}
