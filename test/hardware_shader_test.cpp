...
#if defined(_WIN32)
static bool disassemble_dxbc(const std::string& game_dir,
                             ...
                             std::string& out, std::string& err)
{
    ...
    if (!module) { err = "game dxcompiler.dll is unavailable"; return false; }
    const auto create = reinterpret_cast<DxcCreateInstanceProc>(
        GetProcAddress(module, "DxcCreateInstance"));
    IDxcCompiler3* compiler = nullptr;
    if (!create || FAILED(create(CLSID_DxcCompiler, __uuidof(IDxcCompiler3),
                                 reinterpret_cast<void**>(&compiler))) ||
        !compiler)
    {
        FreeLibrary(module); err = "cannot create IDxcCompiler3"; return false;
    }
    DxcBuffer input{bytecode.data(), bytecode.size(), DXC_CP_ACP};
    IDxcResult* result = nullptr;
    HRESULT hr = compiler->Disassemble(&input, __uuidof(IDxcResult),
    ...
    compiler->Release();
    FreeLibrary(module);
    if (out.empty()) { err = "DXIL disassembly is empty"; return false; }
    return true;
}
#endif
...
int main(int argc, char** argv)
{
    ...
    const bool mounted = src.open(argv[1], err) &&
        (std::string(argv[2]) == "frontend"
            ? src.mount_frontend(err)
            : src.mount_level(argv[2], false, err));
    if (!mounted) {
        std::fprintf(stderr, "mount: %s\n", err.c_str()); return 1;
    }
    MaterialBinding material; bool have_material = false;
    const bool list_matching_depots =
        std::getenv("BF6_SHADER_LIST_MATCHES") != nullptr;
    const char* depot_filter_env = std::getenv("BF6_SHADER_DEPOT_FILTER");
    const std::string depot_filter = depot_filter_env
        ? lower(depot_filter_env) : std::string();
    for (const auto& kv : src.res()) {
        const std::string depot_name = lower(kv.first);
        if (depot_name.find("shaderblockdepot") == std::string::npos) continue;
        if (!depot_filter.empty() && depot_name.find(depot_filter) == std::string::npos)
            continue;
        const std::vector<uint8_t> d = src.get_res(kv.first, err); Depot dep;
        if (d.empty() || !dep.parse(d, err) || !dep.has_key(key)) continue;
        MaterialBinding candidate = dep.textures_for(key, d);
        std::printf("MATERIAL depot=%s constants=%zu textures=%zu\n", kv.first.c_str(),
                    candidate.constants.size(), candidate.textures.size());
        if (!have_material) material = std::move(candidate);
        have_material = true;
        if (!list_matching_depots) break;
    }
    if (!have_material) {
        ...
        for (uint32_t si = 0; si < slots; ++si) {
            ...
            for (uint32_t pi = 0; pi < np && pi < 1024; ++pi) {
                ...
                for (int stage = 0; stage < (pr.size() == 68 ? 2 : 1); ++stage) {
                    ...
                    if (disassemble_dxbc(argv[1], dxbc, disassembly, err))
                    {
                        const std::filesystem::path text_path =
                            outdir / (guid + ".dxil.txt");
                        FILE* tf = nullptr;
                        fopen_s(&tf, text_path.string().c_str(), "wb");
                        if (tf)
                        {
                            fwrite(disassembly.data(), 1, disassembly.size(), tf);
                            fclose(tf);
                        }
                    }
                    else
                        std::fprintf(stderr, "      disassembly: %s\n", err.c_str());
#endif
                }
            }
        }
