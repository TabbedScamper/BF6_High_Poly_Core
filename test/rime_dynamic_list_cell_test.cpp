...
int main(int argc, char** argv)
{
    ...
    /* cl_labelbuttoncell ships IsDisabled -> ConditionalFloat -> Alpha.
     * Exercise that raw graph in both directions.  This guards the shared
     * runtime evaluator: button rows are merely a convenient live oracle, not
     * a special case in the renderer. */
    rime_list::DbdContract button_dbd{};
    const bool button_dbd_ok = rime_list::load_contract(
        ctx, button_dbd, rime_list::kInteractableButtonDbdPartition);
    const rime_list::DbdField* disabled =
        rime_list::contract_field(button_dbd, "IsDisabled");
    rime::Screen button_template{};
    const bool button_screen_ok = load_cell_screen(
        ctx, rime_list::kLabelButtonCellPartition, button_template);
    const int logic_count = bf6_rime_logic_operations(
        ctx, rime_list::kLabelButtonCellPartition, nullptr, 0);
    std::vector<bf6_rime_logic_operation> logic_nodes(
        (size_t)(logic_count > 0 ? logic_count : 0));
    if (logic_count > 0)
        bf6_rime_logic_operations(ctx, rime_list::kLabelButtonCellPartition,
                                  logic_nodes.data(), logic_count);
    int not_nodes = 0;
    for (const bf6_rime_logic_operation& node : logic_nodes)
        not_nodes += node.operation == BF6_RIME_LOGIC_NOT;
    const int fake_logic = bf6_rime_logic_operations(
        ctx, "common/ui/definitely_not_a_real_logic_graph", nullptr, 0);
    std::printf("button-logic-graph\tnodes=%d\tnot=%d\tfake=%d\n",
                logic_count, not_nodes, fake_logic);
    if (button_screen_ok) {
        rime::mark_unresolved_interface_states(ctx, button_template);
        rime::apply_interface_defaults(button_template);
    }
    auto button_alpha = [](const rime::Screen& screen, float& alpha,
                           bool& unresolved) {
        for (const rime::Element& element : screen.elements)
            if (element.name == "CL_PromptButton" &&
                element.partition == rime_list::kLabelButtonCellPartition) {
                alpha = element.alpha;
                unresolved = element.runtime_alpha_unresolved;
                return true;
            }
        return false;
    };
    auto bind_disabled = [&](bool value, rime::Screen& screen,
                             rime_list::BindReport& report) {
        screen = button_template;
        if (!disabled) return;
        rime_list::Record record{};
        record.put(button_dbd, disabled->property_id, bool_value(value));
        report = rime_list::bind_record(
            screen, rime_list::kLabelButtonCellPartition,
            button_dbd, record);
    };
    rime::Screen enabled_button{}, disabled_button{}, shuffled_button = button_template;
    rime_list::BindReport enabled_report{}, disabled_report{}, shuffled_report{};
    bind_disabled(false, enabled_button, enabled_report);
    bind_disabled(true, disabled_button, disabled_report);
    if (disabled) {
        rime_list::Record record{};
        record.put(button_dbd, disabled->property_id, bool_value(false));
        shuffled_report = rime_list::bind_record_shuffled_control(
            shuffled_button, rime_list::kLabelButtonCellPartition,
            button_dbd, record);
    }
    float enabled_alpha = -1.f, disabled_alpha = -1.f;
    bool enabled_unresolved = true, disabled_unresolved = true;
    const bool enabled_label = button_alpha(
        enabled_button, enabled_alpha, enabled_unresolved);
    const bool disabled_label = button_alpha(
        disabled_button, disabled_alpha, disabled_unresolved);
    std::printf("button-alpha-graph\treal=%d/%d\talpha=%.3f/%.3f"
                "\tunresolved=%d/%d\tshuffled=%d/%d\n",
                enabled_report.applied_fields, disabled_report.applied_fields,
                enabled_alpha, disabled_alpha,
                enabled_unresolved ? 1 : 0,
                disabled_unresolved ? 1 : 0,
                shuffled_report.applied_fields,
                shuffled_report.applied_targets);
    const struct { const char* name; bool value; } button_state_probes[] = {
        {"IsSelected", false}, {"IsVisible", true},
        {"IsFocusable", true}, {"IsDisabled", false},
        {"IconVisible", false}, {"IsPremiumColor", false},
        {"HasPayload", false}, {"IsShownPromptIcon", false},
        {"IsHoldButtonEnabled", false}, {"IsActivatedOnButtonUp", true},
        {"IsPrimaryLabelButton", true}, {"DisableMouseInput", false},
        {"UseEffect", true},
    };
    rime_list::Record exact_button_state{};
    int exact_button_routes = 0, shuffled_button_routes = 0;
    for (const auto& probe : button_state_probes) {
        const rime_list::DbdField* field =
            rime_list::contract_field(button_dbd, probe.name);
        if (!field) continue;
        rime_list::Record one{};
        if (!one.put(button_dbd, field->property_id,
                     bool_value(probe.value)))
            continue;
        const rime_list::RouteReport real = rime_list::route_contract(
            ctx, rime_list::kInteractableButtonDbdPartition,
            rime_list::kLabelButtonCellPartition, nullptr, one);
        const rime_list::RouteReport control =
            rime_list::route_contract_shuffled_control(
                ctx, rime_list::kInteractableButtonDbdPartition,
                rime_list::kLabelButtonCellPartition, nullptr, one);
        std::printf("button-state-route\t%s=%d\treal=%d\tshuffled=%d\n",
                    probe.name, probe.value ? 1 : 0,
                    real.connected, control.connected);
        exact_button_routes += real.connected;
        shuffled_button_routes += control.connected;
        if (real.connected > 0 && control.connected == 0)
            exact_button_state.put(button_dbd, field->property_id,
                                   bool_value(probe.value));
    }
    rime::Screen exact_button = button_template;
    const rime_list::BindReport exact_button_binding =
        rime_list::bind_record(exact_button,
            rime_list::kLabelButtonCellPartition, button_dbd,
            exact_button_state);
    rime::solve(exact_button, 1920.f, 48.f);
    std::vector<unsigned char> exact_visible;
    std::vector<float> exact_alpha;
    rime::effective_paint_state(exact_button.elements, exact_visible,
                                exact_alpha);
    int exact_button_paint = 0;
    for (size_t i = 0; i < exact_button.elements.size(); ++i) {
        const rime::Element& element = exact_button.elements[i];
        const bool paint = element.kind == rime::Kind::Label ||
        ...
        if (paint && element.solved && exact_visible[i] &&
            exact_alpha[i] > 0.001f)
            ++exact_button_paint;
    }
    std::printf("button-state-composite\tfields=%zu\troutes=%d/%d\t"
                "bound=%d/%d\tpaint=%d\n",
                exact_button_state.entries().size(), exact_button_routes,
                shuffled_button_routes, exact_button_binding.applied_fields,
                exact_button_binding.supplied, exact_button_paint);
    ok = ok && button_dbd_ok && disabled && button_screen_ok &&
         logic_count > 0 && not_nodes > 0 && fake_logic < 0 &&
         enabled_label && disabled_label &&
         enabled_report.applied_fields == 1 &&
         disabled_report.applied_fields == 1 &&
         std::abs(enabled_alpha - 1.f) < 1e-5f &&
         std::abs(disabled_alpha - 0.25f) < 1e-5f &&
         !enabled_unresolved && !disabled_unresolved &&
         shuffled_report.applied_fields == 0 &&
         shuffled_report.applied_targets == 0;
