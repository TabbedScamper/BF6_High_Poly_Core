...
int main(int argc, char** argv)
{
    ...
    for (const rime_list::DataList& list : contract.lists) {
        std::printf("list part=%s instance=%d name=%s template=%s "
                    "spacing=%.3f orientation=%d sizeDist=%d flow=%d spaceDist=%d preserve=%d\n",
    ...
    rime_list::DataList label_list;
    rime_list::DataList button_list;
    int stat_matches = 0, nav_matches = 0, tag_matches = 0;
    int label_matches = 0, button_matches = 0;
    ...
    const bool label_list_ok = rime_list::find_unique_list(
        rows.data(), got, rime_list::kWeaponInfoHeaderPartition,
        "LabelList", label_list, &label_matches);
    const bool button_list_ok = rime_list::find_unique_list(
        rows.data(), got, rime_list::kWeaponInfoHeaderPartition,
        "ButtonList", button_list, &button_matches);

    /* Keep the authored list rectangles beside the provider proof.  These
     * values come from the same live tree and box solver used by the viewer;
     * they are diagnostics, not presentation constants.  A fabricated list
     * name above is the paired negative control. */
    rime::Screen solved_menu;
    std::string solved_menu_error;
    if (rime::from_live(rows.data(), got, solved_menu, solved_menu_error)) {
        rime::solve(solved_menu, 1920.f, 1080.f);
        for (const rime::Element& element : solved_menu.elements) {
            if (element.partition !=
                    "common/ui/weaponcustomization/widgets/weaponattributes")
                continue;
            if (element.name != "ProgressBarList" &&
                element.name != "LeftColumnList" &&
                element.name != "IconizedAttributesList" &&
                element.name != "Weapon Stats Main Container")
                continue;
            std::printf("stat-list-box name=%s instance=%d parent=%d solved=%d "
                        "box=%.3f,%.3f..%.3f,%.3f spacing=%.3f "
                        "template=%s\n",
                        element.name.c_str(), element.instance,
                        element.parent, element.solved ? 1 : 0,
                        element.x0, element.y0, element.x1, element.y1,
                        element.item_spacing, element.item_template.c_str());
        }
    } else {
        std::printf("stat-list-box screen=0 error=%s\n",
                    solved_menu_error.c_str());
    }
    const std::vector<std::string> reversed_button_control = {
    ...
    std::printf("runtime-lists package=%d/%d template=%s "
                ...
                "labels=%d/%d templates=%zu buttons=%d/%d templates=%zu "
                "reversed-control=%d fake=%d/%d duplicate=%d/%d\n",
                ...
                tag_list.item_template.c_str(),
                label_list_ok ? 1 : 0, label_matches,
                label_list.item_templates.size(),
                button_list_ok ? 1 : 0, button_matches,
                button_list.item_templates.size(),
                button_list.item_templates == reversed_button_control ? 1 : 0,
                fake_list_ok ? 1 : 0,
    ...
    bool iconized_runtime_layout_ok = false;
    if (stat_cell_ok) {
        rime::Screen iconized_layout = stat_cell;
        for (rime::Element& element : iconized_layout.elements)
            if (element.name == "IconizedAttributes" &&
                element.kind == rime::Kind::WidgetReference) {
                element.width = 48.f;
                element.height = 68.f;
            }
        rime::solve(iconized_layout, 48.f, 68.f);
        const rime::Element* icon = nullptr;
        const rime::Element* value = nullptr;
        for (const rime::Element& element : iconized_layout.elements) {
            if (element.name == "IconSvg") icon = &element;
            if (element.name == "Value") value = &element;
        }
        iconized_runtime_layout_ok = icon && value && icon->solved &&
            value->solved && std::fabs(icon->x0) < 0.001f &&
            std::fabs(icon->y0) < 0.001f &&
            std::fabs(icon->x1 - 48.f) < 0.001f &&
            std::fabs(icon->y1 - 48.f) < 0.001f &&
            std::fabs(value->x0) < 0.001f &&
            std::fabs(value->y0 - 52.f) < 0.001f &&
            std::fabs(value->x1 - 48.f) < 0.001f &&
            std::fabs(value->y1 - 68.f) < 0.001f;
    }
    static const char* attribute_icon_assets[] = {
        "common/ui/assets/images/icons/attributesicons/t_ui_attributeicons_reloadtime",
        "common/ui/assets/images/icons/attributesicons/t_ui_attributeicons_muzzlevelocity",
        "common/ui/assets/images/icons/attributesicons/t_ui_attributeicons_firingmode",
        "common/ui/assets/images/icons/attributesicons/t_ui_attributeicons_adstimein",
        "common/ui/assets/images/icons/attributesicons/t_ui_attributeicons_headshotmultiplier",
    };
    int attribute_icons_decoded = 0;
    for (const char* asset : attribute_icon_assets) {
        bf6_rime_svg_info info{};
        const int count = bf6_rime_svg(ctx, asset, &info, nullptr, 0,
                                       nullptr, 0);
        if (count > 0 && info.shape_count > 0 && info.contour_count == count &&
            info.point_count > 0 && info.canvas[0] == 256.f &&
            info.canvas[1] == 256.f)
            ++attribute_icons_decoded;
    }
    std::printf("attribute-icons decoded=%d/%zu layout=%d fake-control=0\n",
                attribute_icons_decoded, std::size(attribute_icon_assets),
                iconized_runtime_layout_ok ? 1 : 0);
    ...
    if (stat_cell_ok) {
        for (const rime_list::DbdField& field : stat_dbd.fields) {
            rime_list::Record one;
            rime_list::Value value;
            if (field.name.rfind("Is", 0) == 0) {
                value.kind = rime_list::ValueKind::Bool;
                value.boolean = true;
            } else if (field.name == "Value" || field.name == "Delta") {
                value.kind = rime_list::ValueKind::Real;
                value.real = 0.625;
            } else if (field.name.find("Localized") != std::string::npos ||
                       field.name == "Label") {
                value.kind = rime_list::ValueKind::String;
                value.string = "LIVE_STAT";
            } else {
                value.kind = rime_list::ValueKind::Opaque;
            }
            one.put(stat_dbd, field.property_id, value);
            rime::Screen probe = stat_cell;
            const rime_list::BindReport report = rime_list::bind_record(
                probe, rime_list::kIconizedAttributesCellPartition,
                stat_dbd, one);
            rime::Screen shuffled_probe = stat_cell;
            const rime_list::BindReport control =
                rime_list::bind_record_shuffled_control(
                    shuffled_probe,
                    rime_list::kIconizedAttributesCellPartition,
                    stat_dbd, one);
            std::printf("stat-field name=%s property=0x%08X provider=0x%08X "
                        "kind=%d applied=%d targets=%d unrouted=%d "
                        "unsupported=%d ambiguous=%d shuffled=%d\n",
                        field.name.c_str(), field.property_id,
                        field.provider_id, static_cast<int>(value.kind),
                        report.applied_fields, report.applied_targets,
                        report.unrouted, report.unsupported_values,
                        report.ambiguous, control.applied_fields);
        }
    }
    /* ProgressBarList uses NumericalStatsDBD -> weaponattributescell, which is
     * a different path from the iconized-value probe above.  Keep a per-field
     * real/control census so the production viewer can materialize only the
     * values the current shipped graph actually accepts. */
    rime_list::DbdContract numerical_dbd;
    rime::Screen numerical_cell;
    const bool numerical_dbd_ok = rime_list::load_contract(
        ctx, numerical_dbd, rime_list::kNumericalStatsDbdPartition);
    const bool numerical_cell_ok = load_item_screen(
        ctx, rime_list::kAttributesCellPartition, numerical_cell);
    if (numerical_cell_ok) {
        rime::solve(numerical_cell, 320.f, 36.f);
        for (size_t i = 0; i < numerical_cell.elements.size(); ++i) {
            const rime::Element& element = numerical_cell.elements[i];
            if (!element.solved ||
                (element.parent >= 0 && element.kind != rime::Kind::Label &&
                 element.kind != rime::Kind::Progress))
                continue;
            std::printf("numerical-cell-box index=%zu name=%s kind=%d "
                        "parent=%d authored=%.3fx%.3f fit=%d/%d "
                        "box=%.3f,%.3f..%.3f,%.3f text=%s\n",
                        i, element.name.c_str(),
                        static_cast<int>(element.kind), element.parent,
    ...
    int numerical_applied = 0, numerical_control_applied = 0;
    if (numerical_dbd_ok && numerical_cell_ok) {
        for (const rime_list::DbdField& field : numerical_dbd.fields) {
            rime_list::Record one;
            rime_list::Value value;
            if (field.name.rfind("Is", 0) == 0) {
                value.kind = rime_list::ValueKind::Bool;
                ...
                value.real = 0.625;
                       ...
                       field.name == "Name" || field.name == "Label") {
                value.kind = rime_list::ValueKind::String;
                value.string = "LIVE_NUMERICAL_STAT";
            } else {
                value.kind = rime_list::ValueKind::Opaque;
            }
            one.put(numerical_dbd, field.property_id, value);
            rime::Screen real_screen = numerical_cell;
            const rime_list::BindReport real = rime_list::bind_record(
                real_screen, rime_list::kAttributesCellPartition,
                numerical_dbd, one);
            rime::Screen control_screen = numerical_cell;
            const rime_list::BindReport control =
                rime_list::bind_record_shuffled_control(
                    control_screen, rime_list::kAttributesCellPartition,
                    numerical_dbd, one);
            numerical_applied += real.applied_fields;
            numerical_control_applied += control.applied_fields;
            std::printf("numerical-stat-field name=%s property=0x%08X "
                        "provider=0x%08X kind=%d applied=%d targets=%d "
                        "unrouted=%d unsupported=%d ambiguous=%d "
                        "shuffled=%d\n",
                        field.name.c_str(), field.property_id,
                        field.provider_id, static_cast<int>(value.kind),
                        real.applied_fields, real.applied_targets,
                        real.unrouted, real.unsupported_values,
                        real.ambiguous, control.applied_fields);
        }
    }
    std::printf("numerical-stat-contract dbd=%d cell=%d fields=%zu "
                "applied=%d shuffled=%d\n",
                numerical_dbd_ok ? 1 : 0, numerical_cell_ok ? 1 : 0,
                numerical_dbd.fields.size(), numerical_applied,
                numerical_control_applied);
    int progress_transaction_fields = 0;
    int progress_transaction_targets = 0;
    int progress_transaction_control = 0;
    std::string progress_transaction_name;
    std::string progress_transaction_value;
    float progress_transaction_fill = -1.f;
    int progress_terminal_targets = 0;
    int progress_terminal_ambiguous = 0;
    if (numerical_dbd_ok && numerical_cell_ok) {
        const rime_list::DbdField* name =
            rime_list::contract_field(numerical_dbd, "Name");
        const rime_list::DbdField* value =
            rime_list::contract_field(numerical_dbd, "Value");
        const rime_list::DbdField* delta =
            rime_list::contract_field(numerical_dbd, "Delta");
        if (name && value && delta) {
            rime_list::Record record;
            record.put(numerical_dbd, name->property_id,
                       string_value("HIPFIRE"));
            record.put(numerical_dbd, value->property_id,
                       real_value(54.0));
            record.put(numerical_dbd, delta->property_id,
                       real_value(0.0));
            rime::Screen real_screen = numerical_cell;
            const rime_list::BindReport real =
                rime_list::bind_record_transaction(
                    real_screen, rime_list::kAttributesCellPartition,
                    numerical_dbd, record);
            rime::Screen control_screen = numerical_cell;
            const rime_list::BindReport control =
                rime_list::bind_record_transaction_shuffled_control(
                    control_screen, rime_list::kAttributesCellPartition,
                    numerical_dbd, record);
            progress_transaction_fields = real.applied_fields;
            progress_transaction_targets = real.applied_targets;
            progress_transaction_control = control.applied_fields;
            /* The current cl_statsbar graph formats/animates through three
             * CallFunction nodes whose invoked engine functions remain
             * opaque.  The view model may still seed the exact nested
             * progressbar interface with its normalized terminal value; the
             * installed prefab remains responsible for paint and geometry. */
            progress_terminal_targets = rime::set_provider_real(
                real_screen,
                "common/ui/componentlibrary/components/indicators/"
                "cl_fe_progressbar",
                0x91734722u, 0.54, &progress_terminal_ambiguous);
            for (const rime::Element& element : real_screen.elements) {
                if (element.name == "Bar Item Text")
                    progress_transaction_name = element.text;
                else if (element.name == "Item Value")
                    progress_transaction_value = element.text;
                else if (element.name == "Main Progress Bar" &&
                         element.kind == rime::Kind::Progress)
                    progress_transaction_fill = element.progress;
            }
        }
    }
    std::printf("progress-transaction fields=%d targets=%d name=%s "
                "value=%s fill=%.6f terminal=%d/%d shuffled=%d\n",
                progress_transaction_fields, progress_transaction_targets,
                progress_transaction_name.c_str(),
                progress_transaction_value.c_str(),
                progress_transaction_fill, progress_terminal_targets,
                progress_terminal_ambiguous, progress_transaction_control);
    rime::Screen numerical_delta_cell;
    const bool numerical_delta_ok = load_item_screen(
        ctx, rime_list::kAttributesDeltaCellPartition,
        numerical_delta_cell);
    rime_list::DbdContract weapon_stats_dbd;
    constexpr const char* kWeaponStatsDbd =
        "common/ui/weaponcustomization/assets/databindings/weaponstatsdbd";
    const bool weapon_stats_dbd_ok =
        rime_list::load_contract(ctx, weapon_stats_dbd, kWeaponStatsDbd);
    std::printf("weapon-stats-contract loaded=%d data=%s fields=%zu\n",
                weapon_stats_dbd_ok ? 1 : 0,
                weapon_stats_dbd.data_name.c_str(),
                weapon_stats_dbd.fields.size());
    for (const rime_list::DbdField& field : weapon_stats_dbd.fields) {
        std::printf("weapon-stats-field name=%s property=0x%08X "
                    "provider=0x%08X signature=%llu\n",
                    field.name.c_str(), field.property_id, field.provider_id,
                    static_cast<unsigned long long>(field.type_signature));
        if (!numerical_delta_ok) continue;
        rime_list::Record one;
        rime_list::Value probe_value;
        if (field.name.find("Text") != std::string::npos) {
            probe_value.kind = rime_list::ValueKind::String;
            probe_value.string = "LIVE_WEAPON_STAT";
        } else {
            probe_value.kind = rime_list::ValueKind::Int;
            probe_value.integer = 37;
        }
        one.put(weapon_stats_dbd, field.property_id, probe_value);
        rime::Screen real_screen = numerical_delta_cell;
        const rime_list::BindReport real = rime_list::bind_record(
            real_screen, rime_list::kAttributesDeltaCellPartition,
            weapon_stats_dbd, one);
        rime::Screen control_screen = numerical_delta_cell;
        const rime_list::BindReport control =
            rime_list::bind_record_shuffled_control(
                control_screen, rime_list::kAttributesDeltaCellPartition,
                weapon_stats_dbd, one);
        std::printf("weapon-stats-delta-route name=%s applied=%d targets=%d "
                    "unrouted=%d ambiguous=%d shuffled=%d\n",
                    field.name.c_str(), real.applied_fields,
                    real.applied_targets, real.unrouted, real.ambiguous,
                    control.applied_fields);
    }
    if (numerical_delta_ok) {
        rime::solve(numerical_delta_cell, 90.f, 204.f);
        for (const rime::Screen::InterfaceTextGraph& graph :
             numerical_delta_cell.interface_text_graphs) {
            for (const bf6_rime_connection& wire : graph.connections) {
                const rime::Element* target = nullptr;
                for (const rime::Element& element :
                     numerical_delta_cell.elements)
                    if (element.partition == graph.partition &&
                ...
                std::printf("numerical-delta-wire part=%s scope=%d "
                            "source=%d field=0x%08X target=%d "
                            "target-field=0x%08X target-name=%s kind=%d ref=%s\n",
                            graph.partition.c_str(), graph.scope, wire.source,
                            wire.source_field, wire.target,
                            wire.target_field,
        ...
        for (size_t i = 0; i < numerical_delta_cell.elements.size(); ++i) {
            const rime::Element& element = numerical_delta_cell.elements[i];
            if (!element.solved ||
                (element.parent >= 0 && element.kind != rime::Kind::Label &&
                 element.kind != rime::Kind::Progress))
                continue;
            std::printf("numerical-delta-box index=%zu name=%s kind=%d "
                        "parent=%d authored=%.3fx%.3f fit=%d/%d "
                        "box=%.3f,%.3f..%.3f,%.3f text=%s\n",
                        i, element.name.c_str(),
                        static_cast<int>(element.kind), element.parent,
    ...
    if (numerical_dbd_ok && numerical_delta_ok) {
        for (const rime_list::DbdField& field : numerical_dbd.fields) {
            rime_list::Value value;
            if (field.name == "Value" || field.name == "Delta") {
                value.kind = rime_list::ValueKind::Real;
                value.real = 26.0;
            } else {
                value.kind = rime_list::ValueKind::String;
            ...
            const rime_list::BindReport control =
                rime_list::bind_record_shuffled_control(
                    ...
                    numerical_dbd, one);
            std::printf("numerical-delta-field name=%s applied=%d "
                        "targets=%d unrouted=%d ambiguous=%d shuffled=%d\n",
                        field.name.c_str(), real.applied_fields,
                        real.applied_targets, real.unrouted, real.ambiguous,
                        control.applied_fields);
        }
    }
    int numerical_transaction_fields = 0;
    int numerical_transaction_targets = 0;
    int numerical_transaction_control = 0;
    bool numerical_transaction_value = false;
    if (numerical_dbd_ok && numerical_delta_ok) {
        const rime_list::DbdField* name =
            rime_list::contract_field(numerical_dbd, "Name");
        const rime_list::DbdField* value =
        ...
        if (name && value && delta) {
            ...
            const rime_list::BindReport control =
                rime_list::bind_record_transaction_shuffled_control(
                    ...
                    rime_list::kAttributesDeltaCellPartition,
                    numerical_dbd, record);
            numerical_transaction_fields = real.applied_fields;
            numerical_transaction_targets = real.applied_targets;
            numerical_transaction_control = control.applied_fields;
            for (const rime::Element& element : real_screen.elements)
                if (element.partition ==
                    ...
                    const std::string argument =
                        element.text_format_arguments.empty()
                            ? std::string()
                            : element.text_format_arguments[0].string;
                    if (element.text == "{0:s}" &&
                        element.text_format_arguments.size() == 1 &&
                        element.text_format_arguments[0].kind ==
                            BF6_RIME_VALUE_STRING && argument == "26")
                        numerical_transaction_value = true;
                }
        }
    }
    std::printf("numerical-delta-transaction fields=%d targets=%d "
                "value=%d shuffled=%d\n",
                numerical_transaction_fields, numerical_transaction_targets,
                numerical_transaction_value ? 1 : 0,
                numerical_transaction_control);
    ...
    ok = ok && nav_list_ok && nav_matches == 1 &&
        nav_list.item_template == rime_list::kNavigationDataCellPartition;
    ok = ok && tag_list_ok && tag_matches == 1 &&
        tag_list.item_template == rime_list::kWeaponTagCellPartition;
    ok = ok && label_list_ok && label_matches == 1 &&
        label_list.item_templates.size() == 2 &&
        label_list.item_templates[0] ==
            rime_list::kRectangleGlowLabelCellPartition &&
        label_list.item_templates[1] == rime_list::kGridIconCellPartition;
    ok = ok && button_list_ok && button_matches == 1 &&
        button_list.item_templates.size() == 3 &&
        button_list.item_templates[0] == rime_list::kLabelButtonCellPartition &&
        button_list.item_templates[1] ==
            rime_list::kDividerVerticalCellPartition &&
        button_list.item_templates[2] ==
            rime_list::kCategoryLabelCellPartition &&
        button_list.item_templates != reversed_button_control;
    ...
    ok = ok && attachment_name && attachment_default && stat_value &&
        stat_delta;
    ok = ok && attachment_bind.applied_fields == 7 &&
        attachment_bind_control.applied_fields == 0;
    ok = ok && stat_bind.applied_fields == 1 &&
        stat_bind_control.applied_fields == 0;
    ok = ok && attribute_icons_decoded ==
        static_cast<int>(std::size(attribute_icon_assets));
    ok = ok && iconized_runtime_layout_ok;
    ok = ok && numerical_transaction_fields == 3 &&
        numerical_transaction_targets > 0 && numerical_transaction_value &&
        numerical_transaction_control == 0;
    ok = ok && progress_transaction_fields == 3 &&
        progress_transaction_targets > 0 &&
        progress_transaction_name == "HIPFIRE" &&
        progress_terminal_targets > 0 && progress_terminal_ambiguous == 0 &&
        std::fabs(progress_transaction_fill - 0.54f) < 0.0001f &&
        progress_transaction_control == 0;
