...
int main(int argc, char** argv)
{
    ...
    // Any normalized mounted EBX path is a valid direct-install probe root.
    // Restricting this to common/ silently redirected game/glacierflow boot
    // probes to the weapon screen and produced convincing output for the
    // wrong asset.
    // argv[2] is normally the recorded TSV oracle and also contains '/'.
    // Treat only normalized mounted asset namespaces as an explicit probe;
    // the old slash-only test accidentally sent the filesystem path to the
    // EBX reader and made the live test fail before testing any Rime row.
    const bool explicitRoot = argc >= 3 &&
        (std::strncmp(argv[2], "common/", 7) == 0 ||
         std::strncmp(argv[2], "game/", 5) == 0);
    const char* root = explicitRoot
    ...
    if (dump_rows)
    {
        std::printf("row\tdepth\tparent\tinstance\tkind\tname\ttype\tpartition\tvisible\tcolor_source\trgb\talpha\timage\tfont\tpoint_size\tline_height\ttext_spacing\ttext_halign\ttext_valign\tword_wrap\toverflow\tx_anchor\tx_offset\ty_anchor\ty_offset\tw\th\tresize\th_align\tv_align\taddress_u\taddress_v\tclip\n");
        for (size_t i = 0; i < rows.size(); ++i)
        {
            const bf6_rime_node& r = rows[i];
            std::printf("%zu\t%d\t%d\t%d\t%d\t%s\t%s\t%s\t%d\t%d\t%06X\t%.6g\t%s\t%s\t%.3f\t%.3f\t%.3f\t%d\t%d\t%d\t%d\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%d\t%d\t%d\t%d\t%d\t%d\n",
                        ...
                        r.line_height, r.text_spacing, r.text_halign,
                        r.text_valign, r.text_word_wrap,
                        r.text_overflow_mode,
                        r.h.anchor_start, r.h.offset_start,
                        r.v.anchor_start, r.v.offset_start, r.width, r.height,
                        r.image_resize_mode, r.image_h_align,
                        r.image_v_align, r.image_address_u,
                        r.image_address_v, r.image_clip_to_bounds);
        }
    }

    const bool dump_strings = argc >= 4 &&
        std::strcmp(argv[3], "--dump-strings") == 0;
    if (dump_strings)
    {
        /* A recursively expanded screen can import the same widget partition
         * many times.  Scan each owning partition once and ask the mounted
         * reader for actual LocalizedStringEntity values; asset names and
         * research tables are deliberately not used as runtime input. */
        std::set<std::string> partitions;
        partitions.insert(root);
        for (const bf6_rime_node& row : rows)
            if (row.partition[0]) partitions.insert(row.partition);
        int real = 0;
        std::printf("partition\tinstance\tsid\ttext\n");
        for (const std::string& partition : partitions)
            for (int instance = 0; instance < 512; ++instance)
            {
                const uint32_t sid = bf6_rime_string_entity(
                    c, partition.c_str(), instance);
                if (!sid) continue;
                const char* text = bf6_localized_string(c, sid);
                std::printf("%s\t%d\t%u\t%s\n", partition.c_str(),
                            instance, sid, text && text[0] ? text : "<missing>");
                ++real;
            }
        int control = 0;
        for (int instance = 0; instance < 512; ++instance)
            control += bf6_rime_string_entity(
                c, "common/ui/__control__/not_a_real_prompt", instance) != 0;
        std::printf("localized-entity real=%d fake-partition-control=%d\n",
                    real, control);
    }
    ...
    const bool dump_events = argc >= 4 &&
        std::strcmp(argv[3], "--dump-events") == 0;
    if (dump_events)
    {
        const int ne = bf6_rime_event_connections(c, root, nullptr, 0);
        std::vector<bf6_rime_event_connection> events(
            (size_t)(ne > 0 ? ne : 0));
        if (ne > 0)
            bf6_rime_event_connections(c, root, events.data(), ne);
        std::printf("source_instance\tsource_event\ttarget_instance\t"
                    "target_event\tmode\n");
        for (const bf6_rime_event_connection& event : events)
            std::printf("%d\t0x%08X\t%d\t0x%08X\t%d\n",
                        event.source, event.source_event, event.target,
                        event.target_event, event.mode);
        std::printf("events=%d\n", ne);
    }
    ...
    if (dump_interfaces)
    {
        ...
        std::printf("interface_instance\tfield_id\taccess\tkind\tbool\tint\tuint\treal\tstring\n");
        for (const bf6_rime_interface_field& field : fields)
            std::printf("%d\t0x%08X\t%d\t%d\t%d\t%lld\t%llu\t%.9g\t%s\n",
    ...
    /* The current-install oracle includes all authored layers and auxiliary
     * mask surfaces. The info view contributes 16 visual rows from its second
     * layer, and the two selected masks contribute four non-final-paint rows
     * (one GlowMask fill plus EmptyDots' container, label and final fill).
     * The former 419 and 435 counts respectively omitted the second layer and
     * then the mask surfaces. */
    static const int kRecordedOracleRows = 439;
