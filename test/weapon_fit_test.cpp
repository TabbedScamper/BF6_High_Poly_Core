...
static std::string find_md(bf6_ctx* c, const char* weapon)
{
    const std::string query = std::string("md_") + weapon;
    const std::string leaf = "/" + query;
    ...
    for (int i = 0; i < got; i++)
    {
        ...
        if (s.find(leaf) == std::string::npos || s.find("_bundle") != std::string::npos)
            continue;
...
static std::string find_m4_md(bf6_ctx* c) { return find_md(c, "m4a1"); }
...
static bool placement_xyz(const Snapshot& s, const char* token, float out[3])
{
    const std::string row = placement_for(s, token);
    if (row.empty()) return false;
    const size_t first = row.find('\t');
    if (first == std::string::npos) return false;
    return std::sscanf(row.c_str() + first + 1, "%f\t%f\t%f",
                       &out[0], &out[1], &out[2]) == 3;
}
...
int main(int argc, char** argv)
{
    ...
    // Charm is a special weapon-space attachment. The model definition's
    // authored Default charm must use the selected receiver's live
    // wep.charmtransform writer, while a zero-optional configured weapon must
    // emit no charm at all. This caught the floating rarebenchmark charm on
    // M1014 that ordinary gameplay-bone tests could not see.
    const std::string m1014_md = find_md(c, "m1014");
    const auto m1014_authored = read_parts(c, m1014_md, nullptr, false);
    const auto m1014_bare = read_parts(c, m1014_md, nullptr, true);
    float charm_anchor[3]{};
    const bool authored_charm = contains_mesh(m1014_authored, "wepchrm") &&
        placement_xyz(m1014_authored, "rarebenchmark", charm_anchor);
    const float charm_anchor_length = std::sqrt(
        charm_anchor[0] * charm_anchor[0] +
        charm_anchor[1] * charm_anchor[1] +
        charm_anchor[2] * charm_anchor[2]);
    const bool charm_uses_receiver_anchor = authored_charm &&
                                            charm_anchor_length > 0.001f;
    const bool bare_has_no_default_charm =
        !contains_mesh(m1014_bare, "wepchrm");
    std::printf("m1014 authored charm row=%s\n",
                placement_for(m1014_authored, "rarebenchmark").c_str());
    std::printf("m1014 default charm anchor=[%.6f %.6f %.6f], live=%d; "
                "zero-optional charm suppressed=%d\n",
                charm_anchor[0], charm_anchor[1], charm_anchor[2],
                charm_uses_receiver_anchor ? 1 : 0,
                bare_has_no_default_charm ? 1 : 0);
    ...
    return real_changed && !fake_changed && fitted.offsets > 0 &&
           ...
           factory_requests_muzzle && factory_has_muzzle &&
           charm_uses_receiver_anchor && bare_has_no_default_charm ? 0 : 1;
}
