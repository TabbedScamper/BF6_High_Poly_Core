...
InstalledSoundResult ReadInstalledSoundConfig(
    ...
    for (const std::string& path : unique) {
        ...
        if (folded.find("/haptics/") != std::string::npos) {
            ++result.controls.config_haptic_imports;
            continue;
        }
        audible.push_back(path);
    }
    if (audible.size() != 1) {
        result.status = InstalledSoundStatus::ConfigWaveAmbiguous;
        result.error = "sound config does not have one audible Wave/Asset import";
        return result;
    }

    InstalledSoundRequest wave_request;
    wave_request.wave_asset = audible.front();
    ...
    result.controls.config_wave_imports = wave_count + asset_count;
    result.controls.config_haptic_imports =
        static_cast<int>(unique.size() - audible.size());
    result.controls.config_xor_import_hits = wave_control + asset_control;
    return result;
}
...
InstalledSoundResult ReadInstalledSoundWave(
    ...
    if (request.metadata_only) {
        // Every control above has already run; only the sample decode is skipped.
...
struct PcmOneShotPlayer::Impl {
    ...
    std::vector<std::unique_ptr<ActiveVoice>> active;
#endif
};
...
#ifdef _WIN32
    if (impl_) {
        for (auto& voice : impl_->active) {
            if (!voice || !voice->source) continue;
            voice->source->Stop();
...
bool PcmOneShotPlayer::Play(const InstalledSoundWave& wave, float volume,
    ...
    // Source voices do not own their submitted bytes. Each active record keeps
    // its own copy alive until XAudio2 reports that the queue drained.
    for (auto it = impl_->active.begin(); it != impl_->active.end();) {
        XAUDIO2_VOICE_STATE state{};
        (*it)->source->GetState(&state, XAUDIO2_VOICE_NOSAMPLESPLAYED);
        if (state.BuffersQueued != 0) {
            ++it;
            continue;
        }
        (*it)->source->DestroyVoice();
        (*it)->source = nullptr;
        it = impl_->active.erase(it);
    }

    // UI sounds are brief, but cap pathological input storms. Retiring the
    // oldest voice here affects only the seventeenth simultaneously queued
    // click and never truncates ordinary fast navigation.
    constexpr size_t kMaxActiveVoices = 16;
    if (impl_->active.size() >= kMaxActiveVoices) {
        auto& oldest = impl_->active.front();
        oldest->source->Stop();
        oldest->source->FlushSourceBuffers();
        oldest->source->DestroyVoice();
        impl_->active.erase(impl_->active.begin());
    }

    auto active = std::make_unique<Impl::ActiveVoice>();
    active->samples = wave.pcm;
    const size_t byte_count = active->samples.size() * sizeof(int16_t);
    ...
    HRESULT hr = impl_->engine->CreateSourceVoice(&active->source, &format);
    if (FAILED(hr)) {
        error = xaudio_error("XAudio2 source voice creation", hr);
        return false;
    }
    ...
    hr = active->source->SetVolume(volume);
    if (SUCCEEDED(hr)) hr = active->source->SubmitSourceBuffer(&buffer);
    if (SUCCEEDED(hr)) hr = active->source->Start();
    if (FAILED(hr)) {
        error = xaudio_error("XAudio2 one-shot submit/start", hr);
        active->source->DestroyVoice();
...
uint32_t PcmOneShotPlayer::ActiveVoiceCount() const {
#ifdef _WIN32
    return impl_ ? static_cast<uint32_t>(impl_->active.size()) : 0;
#else
    return 0;
#endif
}

struct PcmStreamPlayer::Impl {
#ifdef _WIN32
    IXAudio2* engine = nullptr;
    IXAudio2MasteringVoice* mastering = nullptr;
    IXAudio2SourceVoice* source = nullptr;
...
PcmStreamPlayer::PcmStreamPlayer() : impl_(std::make_unique<Impl>()) {}
PcmStreamPlayer::~PcmStreamPlayer() { Stop(); }
...
bool PcmStreamPlayer::Open(uint16_t channels, uint32_t sample_rate,
                           float volume, std::string& error) {
    Stop();
    if (!impl_ || !channels || !sample_rate) {
        error = "stream PCM format is invalid";
        return false;
    }
#ifdef _WIN32
    const HRESULT com = CoInitializeEx(nullptr, COINIT_MULTITHREADED);
    if (SUCCEEDED(com)) impl_->com_owned = true;
        ...
        error = xaudio_error("COM initialization for XAudio2 stream", com);
        return false;
    }
    HRESULT hr = XAudio2Create(&impl_->engine, 0, XAUDIO2_DEFAULT_PROCESSOR);
    if (FAILED(hr)) {
        error = xaudio_error("XAudio2Create stream", hr);
        Stop();
        return false;
    }
    hr = impl_->engine->CreateMasteringVoice(&impl_->mastering);
    if (FAILED(hr)) {
        error = xaudio_error("XAudio2 stream mastering voice creation", hr);
        Stop();
        return false;
    }
    WAVEFORMATEX format{};
    ...
    format.wBitsPerSample = 16;
    format.nBlockAlign = WORD(channels * 2u);
    format.nAvgBytesPerSec = sample_rate * format.nBlockAlign;
    hr = impl_->engine->CreateSourceVoice(&impl_->source, &format);
    if (SUCCEEDED(hr))
        hr = impl_->source->SetVolume(
            (std::max)(0.0f, (std::min)(1.0f, volume)));
    if (FAILED(hr)) {
        error = xaudio_error("XAudio2 stream source voice creation", hr);
        Stop();
        return false;
    }
...
#else
    (void)volume;
    error = "native PCM streaming is available only on Windows";
    return false;
#endif
}

bool PcmStreamPlayer::Submit(const std::vector<int16_t>& pcm,
                             std::string& error) {
#ifdef _WIN32
    if (!impl_ || !impl_->source || pcm.empty() || !impl_->channels ||
        pcm.size() % impl_->channels != 0) {
        error = "stream PCM chunk is empty or not frame-aligned";
        return false;
    }
    XAUDIO2_VOICE_STATE state{};
    impl_->source->GetState(&state, XAUDIO2_VOICE_NOSAMPLESPLAYED);
    while (impl_->buffers.size() > state.BuffersQueued)
        impl_->buffers.pop_front();

    auto chunk = std::make_unique<Impl::Buffer>();
    chunk->samples = pcm;
    const size_t byte_count = chunk->samples.size() * sizeof(int16_t);
    if (byte_count > (std::numeric_limits<UINT32>::max)()) {
        error = "stream PCM chunk exceeds XAudio2 buffer limit";
        return false;
    }
    XAUDIO2_BUFFER buffer{};
    buffer.AudioBytes = static_cast<UINT32>(byte_count);
    buffer.pAudioData = reinterpret_cast<const BYTE*>(chunk->samples.data());
    const HRESULT hr = impl_->source->SubmitSourceBuffer(&buffer);
    if (FAILED(hr)) {
        error = xaudio_error("XAudio2 stream submit/start", hr);
        return false;
    }
    impl_->submitted_frames += pcm.size() / impl_->channels;
    impl_->buffers.push_back(std::move(chunk));
    error.clear();
    return true;
#else
    (void)pcm;
...
bool PcmStreamPlayer::Start(std::string& error) {
#ifdef _WIN32
    if (!impl_ || !impl_->source || impl_->submitted_frames == 0) {
        error = "PCM stream has no submitted audio";
        return false;
    }
    if (impl_->started) {
        error.clear();
    ...
    const HRESULT hr = impl_->source->Start();
    if (FAILED(hr)) {
        error = xaudio_error("XAudio2 stream start", hr);
        return false;
    }
    impl_->started = true;
    error.clear();
...
uint64_t PcmStreamPlayer::SamplesPlayed() const {
#ifdef _WIN32
    if (impl_ && impl_->source) {
        XAUDIO2_VOICE_STATE state{};
        impl_->source->GetState(&state);
...
uint64_t PcmStreamPlayer::SubmittedFrames() const {
    return impl_ ? impl_->submitted_frames : 0;
}

uint64_t PcmStreamPlayer::BufferedFrames() const {
    const uint64_t submitted = SubmittedFrames();
    const uint64_t played = SamplesPlayed();
    return submitted > played ? submitted - played : 0;
}

bool PcmStreamPlayer::IsDrained() const {
    return SubmittedFrames() > 0 && BufferedFrames() == 0;
}
