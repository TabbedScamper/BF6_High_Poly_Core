...
// present. A config may also carry an exact /haptics/ sibling; that authored
// role is counted and excluded from audible PCM. Multiple remaining audible
// imports still fail closed. A one-bit-perturbed field hash is the control.
...
// returns. Independent source voices allow short UI sounds to overlap; dead
// voices are reclaimed on the next Play call or by Stop.
...
// Independent streaming PCM output used by in-memory movie decoding. Submitted
// chunks are copied and retained until XAudio2 drains them; UI one-shots use a
// separate engine/voice pool and therefore cannot stop the movie soundtrack.
class PcmStreamPlayer {
public:
    PcmStreamPlayer();
    ~PcmStreamPlayer();
    PcmStreamPlayer(const PcmStreamPlayer&) = delete;
    PcmStreamPlayer& operator=(const PcmStreamPlayer&) = delete;

    bool Open(uint16_t channels, uint32_t sample_rate, float volume,
              std::string& error);
    bool Submit(const std::vector<int16_t>& pcm, std::string& error);
